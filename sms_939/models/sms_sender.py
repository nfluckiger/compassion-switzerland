...
class SmsSender(models.TransientModel):
    ...
    def send_sms(self):
        ...
        username = tools.config.get('939_username')
        password = tools.config.get('939_password')
        ...
        for sms in self:
            request = "?receiver=%s&text=%s" % (sms.partner_id.mobile,
                                                sms.text)
            request_server.putheader("Authorization", "Basic %s" % auth)
            request_server.request('GET', '/Blue/sms/xml', request)
